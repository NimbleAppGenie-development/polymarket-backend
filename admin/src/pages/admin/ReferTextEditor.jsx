...
export default function ReferDataEdit() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const request = new HttpClient({ url: "/admin/walletConfig", auth: true });
      const { data: resp } = await request.get();
      if (resp?.status) {
        const cfg = resp.data;
        setData({
  ...
  const handleSave = async () => {
    try {
      ...
      const request = new HttpClient({
        url: "/admin/walletConfig/update",
        auth: true,
        data: formData,
        headers: { "Content-Type": "multipart/form-data" },
      });

      const { data: resp } = await request.post();
      ...
      if (resp?.status) {
        ...
        errorToastr(resp?.message || "Failed to update referral data");
      }
    } catch (err) {
      console.error(err);
          ...
          <div className="card card-round">
            ...
            <div className="card-body" style={{ overflowX: "auto" }}>
              ...
              <div className="mb-3" style={{marginTop: "60px"}}>
                <label className="form-label"><strong>Referral Amount</strong></label>
                <input
                  type="text"
              ...
              <div className="mb-3">
                <label className="form-label"><strong>Referral Link</strong></label>
                <input
                  type="text"
                  className="form-control"
              ...
              <div className="mb-3">
                <label className="form-label"><strong>Referral Message</strong></label>
                <textarea
                  className="form-control"
                  rows={3}
                  ...
                  <div key={index} className="mb-3 border p-3 rounded">
                    <div className="mb-2">
                      ...
                      <input
                        ...
                        value={item.heading || ""}
                        onChange={(e) =>
                          handleCardChange("firstObj", index, "heading", e.target.value)
                        }
                      />
                    </div>
                    <div className="mb-2">
                      ...
                      <input
                        ...
                        value={item.text || ""}
                        onChange={(e) =>
                          handleCardChange("firstObj", index, "text", e.target.value)
                        }
                      />
                    </div>
                    <div className="mb-2">
                    ...
                    <button
                      ...
                      onClick={() => removeCardRow("firstObj", index)}
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <div key={index} className="mb-3 border p-3 rounded">
                    <div className="mb-2">
                      ...
                      <input
                        ...
                        value={item.heading || ""}
                        onChange={(e) =>
                          handleCardChange("secondObj", index, "heading", e.target.value)
                        }
                      />
                    </div>
                    <div className="mb-2">
                      ...
                      <input
                        ...
                        value={item.text || ""}
                        onChange={(e) =>
                          handleCardChange("secondObj", index, "text", e.target.value)
                        }
                      />
                    </div>
                    <div className="mb-2">
                    ...
                    <button
                      className="btn btn-sm btn-danger"
                      onClick={() => removeCardRow("secondObj", index)}
                    >
                      Remove
                    </button>
                  </div>
