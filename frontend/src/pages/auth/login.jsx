...
export default function Login() {
    ...
    const handleSubmition = async (e) => {
        ...
        try {
            ...
            if (response.status && response.data?.body) {
                login(response.data.body);
                navigate("/dashboard");
            } else {
                errorToastr(response?.message || response?.data?.message || "Invalid credentials");
            }
        } catch (error) {
            console.error("Login error:", error);

            errorToastr(error?.message || error?.data?.message || "Login failed");
        }

        return false; // prevents form reload
    };
